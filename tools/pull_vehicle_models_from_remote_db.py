...
REMOTE_DB_URL = (
    "https://parseapi.back4app.com/classes/Carmodels_Car_Model_List?order=Year"
)
HEADERS = {
...
def store_data(file_path: Path, data: List[Dict]):
    with open(file_path.joinpath("vehicle-model-dataset.json"), "w+") as f:
        json.dump(data, f)
...
def parse_arguments() -> dict:
    ...
    parser.add_argument("--number", type=int, default=DEFAULT_MODELS_NUMBER)
...
def main():
    ...
    logger.debug(f"data was stored to {file_path} folder as JSON file")
